...
- Review actions run one at a time to prevent duplicate submissions and keep Undo ordering predictable. Errors leave a retry path. Saved-list read/modify/write operations use an origin-wide Web Lock when available to avoid concurrent updates overwriting each other across Tab Out pages.
